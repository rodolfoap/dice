...
static CHIP_LOGIC( 74192_COUNT )
{
    pin[COUNT] = pin[4] & pin[5];
}

static CHIP_LOGIC( 74192_UP )
{
    pin[UP] = pin[5];
}

static CHIP_LOGIC( 74192_DOWN )
{
    pin[DOWN] = pin[4];
}

static CHIP_LOGIC( 74192_A )
{
    if(pin[14])
        pin[3] = 0;
    else if(!pin[11])
...
static CHIP_LOGIC( 74192_B )
{
    if(pin[14])
        pin[2] = 0;
    else if(!pin[11])
        pin[2] = pin[1];
    else if(POS_EDGE_PIN(COUNT) && !pin[UP] && pin[3] && !pin[7])
        pin[2] = prev_pin[2] ^ 1;
    else if(POS_EDGE_PIN(COUNT) && !pin[DOWN] && !pin[3] && (prev_pin[2] || pin[6] || pin[7]))
        pin[2] = prev_pin[2] ^ 1;
    else
        pin[2] = prev_pin[2];
}

static CHIP_LOGIC( 74192_C )
{
    if(pin[14])
        pin[6] = 0;
    else if(!pin[11])
    ...
    else if(POS_EDGE_PIN(COUNT) && !pin[DOWN] && !pin[3] && !pin[2] && (prev_pin[6] || pin[7]))
        pin[6] = prev_pin[6] ^ 1;
    else
        pin[6] = prev_pin[6];
}

static CHIP_LOGIC( 74192_D )
{
    if(pin[14])
        pin[7] = 0;
    else if(!pin[11])
        pin[7] = pin[9];
    else if(POS_EDGE_PIN(COUNT) && !pin[UP] && !(pin[3] && prev_pin[7]))
...
static CHIP_LOGIC( 74192_CO )
{
    pin[12] = (pin[3] & pin[7] & (pin[5] ^ 1)) ^ 1;
}

static CHIP_LOGIC( 74192_BO )
{
...
CHIP_DESC( 74192 ) =
{
    ...
    CHIP_START( 74192_B )
        INPUT_PINS( UP, DOWN, 1, 3, 6, 7, 11, 14, COUNT )
		EVENT_PINS( COUNT )
        OUTPUT_PIN( 2 )
		PREV_OUTPUT_PIN( 2 )
    ...
    CHIP_START( 74192_C )
        INPUT_PINS( UP, DOWN, 2, 3, 7, 10, 11, 14, COUNT )
		EVENT_PINS( COUNT )
        OUTPUT_PIN( 6 )
		PREV_OUTPUT_PIN( 6 )
        OUTPUT_DELAY_NS( 25.0, 29.0 ),
