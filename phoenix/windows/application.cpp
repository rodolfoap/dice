...
static LRESULT CALLBACK Application_windowProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  if(process) switch(msg) {
    ...
    case WM_SIZE: {
      ...
      // Don't update layouts if size hasn't actually changed
      // TODO: Investigate why random WM_SIZE events are being generated?
      // (Possibly because of input polling rate? Increasing the polling rate
      // seems to fix the issue, but leaving this here just in case)
